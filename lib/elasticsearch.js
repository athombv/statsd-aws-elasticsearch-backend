...
var awsReq = require(path.join(__dirname, '../utils/awsReq'));
...
var es_bulk_insert = function elasticsearch_bulk_insert(listCounters, listTimers, listTimerData, listGaugeData) {
      ...
      awsReq(elasticHost, optionsPost, elasticRegion, lg, debug, function(err, res, status, headers) {
        if (err) {
          lg.log('error', 'Error with HTTP request, no stats flushed.');
          console.log(err);
        }
        else if (status > 200) {
          var errdata = "HTTP " + status + ": " + res;
...
exports.init = function elasticsearch_init(startup_time, config, events, logger) {
  ...
  elasticRegion         = configEs.region         || 'eu-west-1';
